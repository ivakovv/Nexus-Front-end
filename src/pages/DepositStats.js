...
function DepositStats() {
  ...
  const fetchDeposits = async () => {
    try {
      ...
      response.data.forEach(dep => {
        if (!dep.датаОткрытия || !dep.суммаДепозита) return;
        const date = new Date(dep.датаОткрытия);
        const month = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
        if (!grouped[month]) grouped[month] = 0;
        grouped[month] += Number(dep.суммаДепозита);
      });
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4 }}>
        ...
        <ResponsiveContainer>
          <LineChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
            ...
            <Line
              ...
              stroke="#2196f3"
              strokeWidth={3}
              dot={{ fill: '#2196f3', strokeWidth: 2, r: 4, opacity: showAnimation ? 1 : 0 }}
      ...
      <Button variant="outlined" onClick={() => navigate('/deposits')} sx={{ mt: 2 }}>
        Вернуться к вкладам
      </Button>
    </Container>
  );
